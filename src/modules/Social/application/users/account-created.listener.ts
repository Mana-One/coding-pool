...
export class AccountCreatedListener {
    ...
    async create(event: AccountCreated) {
        ...
        await session.run(
            "CREATE (u:User { id: $id, username: $username, memberSince: $memberSince, programs: 0, competitions_entered: 0, competitions_won: 0})",
            { id: event.id, username: event.username, memberSince: types.DateTime.fromStandardDate(new Date()) }
        )
